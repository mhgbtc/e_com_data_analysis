...
| 2026-09-04 | Optimisations + MainApp (Parties 5-6) | Membre C | Membre B | Le cache est piloté par la config, ce qui permet de comparer avec et sans optimisation. Bonne distinction : le DataFrame enrichi (le plus gros) est persisté en mémoire et sur disque, alors que la matrice de cohortes (plus petite) est simplement mise en cache, et les deux sont libérés avec unpersist à la fin. Le MainApp est bien organisé : chaque phase est chronométrée et le try/catch/finally garantit que spark.stop() est appelé même en cas d'erreur. Petite remarque : l'étape "analytics" n'a pas de branchement séparé, donc la lancer revient à lancer tout le pipeline. Rien de bloquant, OK. |
